...
export class BillingSummaryComponent implements OnInit {
  ...
  drawFirstPageLayout(doc: any, pageWidth: number,pagefor:string): void {
    ...
    doc.text(pagefor=="taxinvoice"?"TAX INVOICE":"PACKING LIST", pageWidth / 2, nextY + 5, { align: "center" });
...
exportTaxInvoiceToPDF(): void {
  ...
  tableBody.push([
    ...
    {
      content: "Total",
      styles: { halign: "right" as const, fontStyle: "bold" as const, fillColor: [240, 240, 240] }
    },
    {
  ...
  autoTable(doc, {
    ...
    didParseCell: function(data) {
      ...
      else if (data.row.index >= emptyRowStart && data.row.index <= emptyRowEnd) {
        if (data.cell.text[0] === "") {
  ...
  body: [
    ...
    [
      ...
      },
      {
        ...
        styles: { halign: "right" as const, valign: "bottom" as const, minCellHeight: 20 }
      }
    ],
    ...
    [
      {
        content: "We declare that this invoice shows the actual price of the goods described and that all particulars are true and correct.",
        colSpan: 2,
